...
fileibdal1=args.ibdallele+"/trim0_ibdal1s_"+str(chr)+".csv.gz"
mafpath=args.maf+"/chr"+str(chr)+".frq"
outibd1=args.out+"/trim"+str(trim)+"_al1s_"+str(chr)+".csv.gz"

print ('IBD1 status file',fileibd1)
print ('IBD1 shared allele file', fileibdal1)
...
#Use maf information to impute the "shared" allele when we were unable to infer the shared allele from phasing information
...
for j in range(0,S.shape[0]):
        S.iloc[j,:]=np.where(abs(S.iloc[j,:])>500,1-meang.iloc[j],S.iloc[j,:])
...
for j in range(0,names.shape[0]):
        names1[j]=names[j].split("_")[0]+"_ibd1s_"+names[j].split("_")[1]
        names2[j]=names[j].split("_")[1]+"_ibd1s_"+names[j].split("_")[0]
